...
class BranchAdmin(NestedModelAdmin):
    ...
    readonly_fields = (
        "yandex_review_count_computed",
        "yandex_review_avg_computed",
        "twogis_review_count_computed",
        "twogis_review_avg_computed",
        "vlru_review_count_computed",
        "vlru_review_avg_computed",
    )
    ...
    def get_urls(self):
        ...
        my_urls = [
            path(
                ...
                self.admin_site.admin_view(self.parsing),
            ),
            path(
                "<path:object_id>/change/parse-yandex/",
                self.admin_site.admin_view(self.parsing_yandex),
            ),
            path(
                "<path:object_id>/change/parse-2gis/",
                self.admin_site.admin_view(self.parsing_2gis),
            ),
            path(
                "<path:object_id>/change/parse-vlru/",
                self.admin_site.admin_view(self.parsing_vlru),
            ),
        ]
        return my_urls + urls
...
for provider, field_prefix, label_prefix in (
    ...
    setattr(
        BranchAdmin,
        f"{field_prefix}_review_count_computed",
        _make_provider_metric(
            provider, "count", f"{label_prefix} review count"
        ),
    )
    setattr(
        BranchAdmin,
        f"{field_prefix}_review_avg_computed",
        _make_provider_metric(provider, "avg", f"{label_prefix} review avg"),
    )
